  ...
  return (
    <div className="bg-stone-950 pt-24 pb-20 min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {categories.map(cat => (
              <button
                ...
                className={`px-6 py-2 rounded-full text-sm font-semibold transition-all duration-300 transform hover:scale-105 active:scale-95
                  ${filter === cat 
                    ? 'bg-gold-500 text-stone-950 shadow-lg shadow-gold-500/20' 
        ...
        {/* Animation Wrapper Key ensures animation triggers on filter change */}
        <div key={filter} className="grid grid-cols-1 lg:grid-cols-2 gap-12 animate-fade-in">
          {filteredProjects.map((project) => (
            <div key={project.id} className="group bg-stone-900 rounded-2xl overflow-hidden border border-stone-800 hover:border-gold-500/30 transition-all duration-500 hover:shadow-2xl hover:shadow-gold-500/10 hover:-translate-y-1">
              ...
              <div className="relative h-80 overflow-hidden">
                ...
                {/* Hover Overlay */}
                <div className="absolute inset-0 bg-stone-950/80 opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-center justify-center p-6 text-center backdrop-blur-sm">
                ...
                <div className="absolute top-4 right-4 bg-black/60 backdrop-blur text-white text-xs px-3 py-1 rounded-full border border-white/10 shadow-lg">
                  {project.category}
                </div>
              </div>
        ...
        {filteredProjects.length === 0 && (
           <div className="text-center py-20 text-stone-500 animate-fade-in">
             <p>Za to kategorijo trenutno ni projektov.</p>
           </div>
        )}
      </div>
    </div>
